...
class AppAPI:
    ...
    def load_model(self, path, ctx, gpu):
        return self.engine.load(path, ctx, gpu)
    ...
    def toggle_shared_context(self, enabled):
        self.engine.shared_context = enabled
        return True
    ...
    def stop_recording(self):
        return self.audio.stop_and_transcribe()
    ...
    def pick_document(self):
        ...
        if result and len(result) > 0:
            ...
            text, tokens = self.doc_processor.process(path, self.engine)
    ...
    def _process_message(self, suite, text, persona, context, msg_id):
        full_text = ""
        for chunk in self.engine.generate_stream(suite, text, persona, context, self.current_document):
            chunk = chunk.replace("*", "").replace("_", "")
            full_text += chunk
            safe_chunk = chunk.replace("\\", "\\\\").replace("`", "\\`").replace("$", "\\$").replace("\n", "\\n").replace('"', '\\"')
            if self.window:
                self.window.evaluate_js(f'update_stream("{msg_id}", "{safe_chunk}");')
        ...
        if self.window:
            self.window.evaluate_js(f'finalize_stream("{msg_id}", "{safe_html}", "{safe_raw}");')
    ...
    def save_text_file(self, content):
        ...
        result = self.window.create_file_dialog(webview.SAVE_DIALOG, save_filename='note.txt', file_types=file_types)
        if result:
            try:
    ...
    def clear_chat_history(self, suite="procomm"):
        self.engine.clear_history(suite)
        return True
    ...
    def load_chat_file(self, suite="procomm"):
        ...
        if result and len(result) > 0:
            try:
                with open(result[0], 'r', encoding='utf-8') as f:
                    history = json.load(f)
                    self.engine.set_history(history, suite)
                    return history
            except Exception as e:
    ...
    def save_chat_file(self, suite="procomm"):
        file_types = ('JSON Files (*.json)', 'All files (*.*)')
        result = self.window.create_file_dialog(webview.SAVE_DIALOG, save_filename='chat_history.json', file_types=file_types)
        if result:
            try:
                with open(result, 'w', encoding='utf-8') as f:
                    json.dump(self.engine.get_history(suite), f, indent=4)
                return True
            except Exception as e:
